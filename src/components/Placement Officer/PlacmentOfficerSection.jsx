...
import { Plus, Users, Briefcase, Star, TrendingUp, Award, Eye, CheckCircle, Clock, Filter, Search } from 'lucide-react'
import { useNavigate } from 'react-router-dom'
const PlacementOfficerSection = () => {
  ...
  {/* Optional light background blobs (very subtle) */}
  <div className="fixed inset-0 overflow-hidden pointer-events-none">
    <div className="absolute -top-40 -right-40 w-80 h-80 bg-purple-200 rounded-full filter blur-3xl opacity-20"></div>
    <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-cyan-200 rounded-full filter blur-3xl opacity-20"></div>
    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-pink-200 rounded-full filter blur-3xl opacity-10"></div>
  </div>
    ...
    <div className="mb-12">
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          ...
          className="group relative overflow-hidden bg-gradient-to-r from-purple-500 via-pink-500 to-blue-500 text-white px-8 py-4 rounded-2xl font-bold text-lg transition-all duration-300 transform hover:scale-105 hover:shadow-lg"
        >
      ...
      {isLoading ? (
        ...
        <div className="grid gap-6">
          {filteredApplications.map((app, index) => (
            <div
              key={app.id || index}
              className="group relative bg-white border border-gray-200 rounded-2xl p-6 hover:shadow-lg transition-all duration-500 hover:scale-[1.02]"
            >
              {/* Decorative Elements (lighter) */}
              <div className="absolute top-0 right-0 w-32 h-32 bg-purple-100 rounded-bl-full opacity-20 group-hover:opacity-40 transition-opacity duration-300"></div>
              ...
              <div className="relative z-10 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 bg-gradient-to-br from-purple-500 to-pink-500 rounded-2xl flex items-center justify-center font-bold text-white text-xl shadow-md">
...
export default PlacementOfficerSection